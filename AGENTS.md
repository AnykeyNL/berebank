...
| `get_fable5_analysis` | Fable5 direction outlook for a market (same ranges as `analyze_market`): eight fixed-weight signals — the five strategies plus dual-horizon momentum, a slow stochastic, and ADX trend strength — blended into one bullish/bearish/neutral verdict with a -100..+100 gauge score, weighted-agreement confidence, and per-strategy votes. Includes a `track_record` (hit rate of past outlooks over the following 5 days) once enough daily history has been harvested. Same engine as the web app's Fable5 page |
